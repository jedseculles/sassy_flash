...
Rails 3.0.x plugin that provides "sassy" flash messages. Base code are from Ryan Heath's <a src='http://github.com/rpheath/pretty_flash'>PrettyFlash</a>.
...
<pre><code>$ rake sassy_flash:install[jquery]
$ rake sassy_flash:install[mootools]
$ rake sassy_flash:install[prototype]</code></pre>
  
Right now, this plugin supports jQuery, MooTools, and Prototype. If you're not using either, feel free to add a code similar to behavior of the existing javascripts and ask for a pull request. ;-)

These javascripts are only used to fade the flash messages after some seconds (defaults to 2.5 seconds).
